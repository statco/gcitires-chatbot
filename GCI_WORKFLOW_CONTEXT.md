...
| **gcitires-chatbot** | Customer-facing AI chat widget embedded on the storefront. Uses Airtable (via gci-brain's proxy) for customer memory/conversation history. | `gcitires-chatbot.vercel.app` | ✅ mostly working. ⛔ `/api/memory` times out regularly (15s limit, Airtable too slow for this — needs migration to a real DB, not yet done). |
...
| **Airtable** (`GCI Installer Portal` base) | gci-brain (owns the proxy at `/api/airtable`), gcitires-chatbot, gci-order-hub (calls gci-brain's proxy) | Server-side API key, proxied through gci-brain | ⛔ **`/api/airtable` has no caller authentication and open CORS** — anyone who finds the endpoint can read Installer PII (bank info) or write fake records. Known issue, fix planned post-audit-report. Don't add more callers to this endpoint until it's locked down. |
